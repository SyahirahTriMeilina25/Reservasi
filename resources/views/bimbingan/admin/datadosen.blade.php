...
@push('styles')
<style>
    /* Action Icons - sama seperti data mahasiswa */
    .action-icons {
        display: flex;
    ...
    /* Search Container Styling - sama persis dengan data mahasiswa */
    .search-container {
        position: relative;
        width: 100%;
    ...
    /* Pagination styles - sama dengan data mahasiswa */
    .pagination {
        margin-bottom: 0;
    }
...
<div class="container mt-5">
    <h1 class="mb-2 gradient-text fw-bold">Data Dosen</h1>
    <hr>
    <button class="btn btn-gradient mb-4 mt-2 d-flex align-items-center justify-content-center">
        <a href="{{ route('admin.tambahdosen') }}">
    ...
    <div class="card shadow-lg border-0 rounded-4 overflow-hidden">
        <div class="card-body p-4">
            <h5 class="mb-3 fw-bold">Data Dosen</h5>
            <hr class="mt-0 mb-3">
                ...
                <table class="table table-striped table-bordered align-middle">
                    <thead class="text-center">
                        <tr>
                            ...
                            <th scope="col" class="text-center align-middle">Nama Singkat</th>
                    ...
                    <tbody>
                        ...
                        <tr class="text-center">
                            ...
                            <td>
                                <span class="badge {{ $dosen->role && $dosen->role->role_akses == 'koordinator_prodi' ? 'bg-success' : 'bg-primary' }}">
                                    {{ $dosen->role ? ucfirst(str_replace('_', ' ', $dosen->role->role_akses)) : '-' }}
                                </span>
                            </td>
                            <td>
                                <div class="action-icons">
                                    <a href="{{ route('admin.editdosen', $dosen->nip) }}" 
                                    ...
                                    <button type="button" 
                                            ...
                                            onclick="showResetConfirmation('{{ $dosen->nip }}', '{{ $dosen->nama }}')">
                                        <i class="bi bi-key"></i>
                                    </button>
                                    <button type="button" 
                                            ...
                                            data-bs-toggle="tooltip"
                                            title="Hapus Dosen"
                                            onclick="showDeleteConfirmation('{{ $dosen->nip }}', '{{ $dosen->nama }}')">
            ...
            @if($dosens instanceof \Illuminate\Pagination\LengthAwarePaginator && $dosens->total() > 0)
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-center mt-3">
                <p class="mb-3 mb-lg-0">
                    Menampilkan {{ $dosens->firstItem() ?? 0 }} sampai {{ $dosens->lastItem() ?? 0 }} 
                    dari {{ $dosens->total() ?? 0 }} entri
                </p>
                <nav aria-label="Page navigation">
                    <ul class="pagination mb-0 justify-content-center justify-content-lg-end">
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link" href="{{ $dosens->appends(request()->query())->previousPageUrl() }}" rel="prev">« Sebelumnya</a>
                            </li>
                        @endif

                        {{-- Pagination Elements --}}
                        @foreach ($dosens->appends(request()->query())->getUrlRange(1, $dosens->lastPage()) as $page => $url)
                        ...
                        @if ($dosens->hasMorePages())
                            <li class="page-item">
                                <a class="page-link" href="{{ $dosens->appends(request()->query())->nextPageUrl() }}" rel="next">Selanjutnya »</a>
                            </li>
                        @else
                            <li class="page-item disabled">
...
<!-- HTML untuk Modal Reset Password Dosen (diperbaiki) -->
            ...
            <div class="modal-body py-4">
                <div class="d-flex flex-column align-items-center">
                    ...
                    <div class="text-center">
                        ...
                        <p class="mb-0 text-secondary small" id="resetPasswordText">Reset password dosen ...</p>
                        <p class="mb-0 mt-2 text-warning small">Password akan direset ke NIP dosen</p>
                    </div>
                </div>
            </div>
...
<!-- HTML untuk Modal Hapus Dosen (diperbaiki) -->
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (confirmResetBtn) {
            confirmResetBtn.addEventListener('click', function() {
                if (currentAction === 'reset' && currentNip) {
                    handleResetPassword(currentNip, currentNama);
                }
            });
        }
        ...
        if (confirmDeleteBtn) {
            confirmDeleteBtn.addEventListener('click', function() {
                if (currentAction === 'delete' && currentNip) {
                    handleDelete(currentNip, currentNama);
                }
            });
        }
        
        // Fungsi global untuk menampilkan modal reset password
        window.showResetConfirmation = function(nip, nama) {
            currentAction = 'reset';
            currentNip = nip;
            currentNama = nama;
            
            document.getElementById('resetPasswordText').textContent = `Reset password dosen "${nama}"?`;
            const resetModal = new bootstrap.Modal(document.getElementById('resetPasswordModal'));
            resetModal.show();
        };
        
        // Fungsi global untuk menampilkan modal delete
        window.showDeleteConfirmation = function(nip, nama) {
            currentAction = 'delete';
            currentNip = nip;
            currentNama = nama;
            
            document.getElementById('deleteText').textContent = `Hapus data dosen "${nama}"?`;
            const deleteModal = new bootstrap.Modal(document.getElementById('deleteModal'));
            deleteModal.show();
        };
        
        // Fungsi untuk handle reset password dengan AJAX
        function handleResetPassword(nip, nama) {
            ...
            const url = `{{ route('admin.resetpassworddosen', '') }}/${nip}`;
            ...
            .then(data => {
                ...
                if (data.success) {
                    showSuccessAlert(
                        ...
                        data.message || `Password dosen "${nama}" telah berhasil direset ke NIP.`
                    );
                } else {
                    showErrorAlert(
        ...
        function handleDelete(nip, nama) {
            ...
            const url = `{{ route('admin.hapusdosen', '') }}/${nip}`;
            ...
            .then(data => {
                ...
                if (data.success) {
                    showSuccessAlert(
                        ...
                        data.message || `Data dosen "${nama}" telah berhasil dihapus.`
                    );
                } else {
                    showErrorAlert(
    ...
    // Fungsi pencarian table (tetap sama seperti sebelumnya)
