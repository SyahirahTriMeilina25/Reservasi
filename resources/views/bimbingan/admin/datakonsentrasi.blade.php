...
<div class="container mt-5">
    <h1 class="mb-2 gradient-text fw-bold">Data Konsentrasi</h1>
    <hr>
    <button class="btn btn-gradient mb-4 mt-2 d-flex align-items-center justify-content-center">
        <a href="{{ route('admin.tambahkonsentrasi') }}">
    ...
    <div class="card shadow-lg border-0 rounded-4 overflow-hidden">
        <div class="card-body p-4">
            <h5 class="mb-3 fw-bold">Data Konsentrasi</h5>
            <hr class="mt-0 mb-3">
                ...
                <table class="table table-striped table-bordered align-middle">
                    <thead class="text-center">
                        <tr>
                            ...
                            <th scope="col" class="text-center align-middle">Nama Konsentrasi</th>
                            <th scope="col" class="text-center align-middle">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($konsentrasis as $index => $konsentrasi)
                        <tr class="text-center">
                            <td>{{ ($konsentrasis->currentPage() - 1) * $konsentrasis->perPage() + $loop->iteration }}</td>
                            <td>{{ $konsentrasi->nama_konsentrasi }}</td>
                            <td>
                                <div class="action-icons">
                                    <a href="{{ route('admin.editkonsentrasi', $konsentrasi->id) }}" 
                                    ...
                                    <button type="button" 
                                            ...
                                            onclick="showDeleteConfirmation('{{ $konsentrasi->id }}', '{{ $konsentrasi->nama_konsentrasi }}')">
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="text-center">Belum ada data konsentrasi</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if($konsentrasis instanceof \Illuminate\Pagination\LengthAwarePaginator && $konsentrasis->total() > 0)
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-center mt-3">
                <p class="mb-3 mb-lg-0">
                    Menampilkan {{ $konsentrasis->firstItem() ?? 0 }} sampai {{ $konsentrasis->lastItem() ?? 0 }} 
                    dari {{ $konsentrasis->total() ?? 0 }} entri
                </p>
                <nav aria-label="Page navigation">
                    <ul class="pagination mb-0 justify-content-center justify-content-lg-end">
                        {{-- Previous Page Link --}}
                        @if ($konsentrasis->onFirstPage())
                            <li class="page-item disabled">
                                <span class="page-link">« Sebelumnya</span>
                            </li>
                        @else
                            <li class="page-item">
                                <a class="page-link" href="{{ $konsentrasis->appends(request()->query())->previousPageUrl() }}" rel="prev">« Sebelumnya</a>
                            </li>
                        @endif

                        {{-- Pagination Elements --}}
                        @foreach ($konsentrasis->appends(request()->query())->getUrlRange(1, $konsentrasis->lastPage()) as $page => $url)
                            @if ($page == $konsentrasis->currentPage())
                        ...
                        {{-- Next Page Link --}}
                        @if ($konsentrasis->hasMorePages())
                            <li class="page-item">
                                <a class="page-link" href="{{ $konsentrasis->appends(request()->query())->nextPageUrl() }}" rel="next">Selanjutnya »</a>
                            </li>
                        @else
                            <li class="page-item disabled">
...
<!-- HTML untuk Modal Hapus Konsentrasi (diperbaiki) -->
            ...
            <div class="modal-body py-4">
                <div class="d-flex flex-column align-items-center">
                    ...
                    <div class="text-center">
                        ...
                        <p class="mb-0 text-secondary small" id="deleteText">Hapus data konsentrasi ...</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (confirmDeleteBtn) {
            confirmDeleteBtn.addEventListener('click', function() {
                if (currentAction === 'delete' && currentId) {
                    handleDelete(currentId, currentNama);
                }
            });
        }
        
        // Fungsi global untuk menampilkan modal delete
        window.showDeleteConfirmation = function(id, nama) {
            currentAction = 'delete';
            currentId = id;
            currentNama = nama;
            
            document.getElementById('deleteText').textContent = `Hapus data konsentrasi "${nama}"?`;
        ...
        function handleDelete(id, nama) {
            ...
            const url = `{{ route('admin.hapuskonsentrasi', '') }}/${id}`;
            ...
            .then(data => {
                ...
                if (data.success) {
                    showSuccessAlert(
                        ...
                        data.message || `Data konsentrasi "${nama}" telah berhasil dihapus.`
                    );
                } else {
                    showErrorAlert(
